...
(function () {
	...
	w2grid.prototype = {
		...
		buttons: {
			'reload'	: { type: 'button', id: 'reload', img: 'icon-reload', hint: 'Recarga la información' },
			'columns'	: { type: 'drop', id: 'column-on-off', img: 'icon-columns', hint: 'Muestra/Oculta columnas', arrow: false, html: '' },
			...
			'search-go'	: { type: 'check',  id: 'search-advanced', caption: 'Buscar...', hint: 'Abre los campos de busqueda' },
			'add'		: { type: 'button', id: 'add', caption: 'Agregar', hint: 'Agrega un nuevo registro', img: 'icon-add' },
            'QR'		: { type: 'check', id: 'QR', caption: 'Lector QR', hint: 'Lectura de Codigo QR', img: 'icon-QR', checked: false, disabled: true },
			'edit'		: { type: 'button', id: 'edit', caption: 'Editar', hint: 'Edita un registro', img: 'icon-edit', disabled: true },
			'delete'	: { type: 'button', id: 'delete', caption: 'Borrar', hint: 'Borra los registros seleccionados', img: 'icon-delete', disabled: true },
			'save'		: { type: 'button', id: 'save', caption: 'Grabar', hint: 'Graba los registros cambiados', img: 'icon-save' }
		},
		...
		selectAll: function () {
            ...
            // nuevo, disable QR propiedad de ser presionable al momento de seleccionar elemento en grid
            //if (sel.length == 1) this.toolbar.enable('QR'); else this.toolbar.disable('QR');
		...
		refresh: function () {
			...
			if (window.getSelection) window.getSelection().removeAllRanges(); // clear selection
            // nuevo, disable QR propiedad de ser presionable al momento de seleccionar elemento en grid
			this.toolbar.disable('edit', 'delete', 'QR');
            //this.toolbar.disable('edit', 'delete');
			if (!this.box) return;
			// event before
		...
		initToolbar: function () {
			...
			if (typeof this.toolbar['render'] == 'undefined') {
				...
				if (this.show.toolbarSearch && (this.show.toolbarAdd || this.show.toolbarQR || this.show.toolbarEdit || this.show.toolbarDelete || this.show.toolbarSave)) {
					this.toolbar.items.push({ type: 'break', id: 'break1' });
				}
				if (this.show.toolbarAdd) {
					this.toolbar.items.push($.extend(true, {}, this.buttons['add']));
				}
                if (this.show.toolbarQR) {
					this.toolbar.items.push($.extend(true, {}, this.buttons['QR']));
				}
				...
				if (this.show.toolbarSave) {
					if (this.show.toolbarAdd || this.show.toolbarQR ||this.show.toolbarDelete || this.show.toolbarEdit) {
				...
				this.toolbar.on('click', function (event) {
					...
					switch (id) {
                        ...
                        case 'QR':
							// events
							var eventData = obj.trigger({ phase: 'before', target: obj.name, type: 'QR', recid: null });
							obj.trigger($.extend(eventData, { phase: 'after' }));
							break;
						case 'edit':
							var sel 	= obj.getSelection();
...
var w2alert = function (msg, title, callBack) {
	...
	if ($('#w2ui-popup').length > 0) {
		...
		w2popup.open({
			width 	: 450,
			height 	: 200,
			//showMax : false,      // se quito para que de bloqueo y no cierre popup
			title 	: title,
			modal		: true,     // se agrego para que no cierre popup
			showClose	: false,    // se agrego para que no cierre popup
	...
	if ($('#w2ui-popup').length > 0) {
		w2popup.message({
			...
			html 	: '<div style="position: absolute; top: 0px; left: 0px; right: 0px; bottom: 40px; overflow: auto">'+
					  ...
					  '</div>'+
					  '<div style="position: absolute; bottom: 7px; left: 0px; right: 0px; text-align: center; padding: 5px">'+
                      '		<input id="Yes" type="button" value="'+ w2utils.lang('Si') +'" class="w2ui-popup-button">'+  // test boton si
		...
		w2popup.open({
			...
			buttons 	: '<input id="No" type="button" value="'+ w2utils.lang('No') +'" class="w2ui-popup-button">'+
					  	  '<input id="Yes" type="button" value="'+ w2utils.lang('Si') +'" class="w2ui-popup-button">',
                          //'<input id="Yes" type="button" value="'+ w2utils.lang('Yes') +'" class="w2ui-popup-button">',
						...
						$(this) // remove transtion needed for functionality
							...
							.on('blur', function (event) {
								...
								if ($.trim($(obj).val()) != '' && !w2utils.isDate($(obj).val(), options.format)) {
									$(this).w2tag(w2utils.lang('No es una fecha valida') + ': '+ options.format);
								}
								clearInterval($(obj).data('mtimer'));
								$('#global_calendar_div').remove();
							})
				...
				switch (field.type) {
					...
					case 'date':
						...
						if (this.record[field.name] && !w2utils.isDate(this.record[field.name], field.options.format)) {
							errors.push({ field: field, error: w2utils.lang('No es una fecha valida') + ': ' + field.options.format });
						} else {
