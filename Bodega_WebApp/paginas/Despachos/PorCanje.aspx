<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Site.Master"
 CodeBehind="PorCanje.aspx.vb" Inherits="Bodega_WebApp.PorCanje" %>

<asp:Content ID="HeaderContent" runat="server" ContentPlaceHolderID="HeadContent">
    <%  CType(Me.Page.Master, Bodega_WebApp.Site).idePagina = Bodega_WebApp.Pagina.despaCanje%>
</asp:Content>
...
<asp:Content ID="scriptCotent" ContentPlaceHolderID="contenedorJavascript" runat="server">
    <script type="text/javascript">
         ...
         *Correcciones:
            ...
            Boton grabar
            El save detalle mov se saco de donación porque se descuenta la existenia del material que se retorno
        ...
        $('#grid').w2grid({
            ...
            columns: [
                    ...
                    { field: 'loteSerie', caption: 'Serie o Lote', size: '16%',
                        editable: { type: 'text', inTag: 'maxlength=40' }, attr: "align=center"
                    },
                ...
                if (event.column == 3) {
                    ...
                    if (event.recid == "") {
                        ...
                        if (event.value_new >= 1) {
                            ...
                            if (event.value_new <= local_Existencia) {
                                local_Cantidad = event.value_new
                            } else {
                                alert('¡Error, el valor no puede superar la existencia del producto.!');
                                local_Cantidad = 1;
                            }
                            ...
                            if (local_Fecha == "" || local_Serie == "") {
                                w2ui['grid'].add({ recid: event.recid, codMaterial: local_Codigo, nombreMaterial: local_nombre, item: local_Item, cantidad: local_Cantidad, valor: local_Valor, total: total, existencia: local_Existencia, loteSerie: local_Serie, fechaVencimiento: local_Fecha });
                            } else {
                                w2ui['grid'].add({ recid: event.recid, codMaterial: local_Codigo, nombreMaterial: local_nombre, item: local_Item, cantidad: local_Cantidad, valor: local_Valor, total: total, existencia: local_Existencia, loteSerie: local_Serie, fechaVencimiento: local_Fecha });
                ...
                if (event.column == 7) {
                    ...
                    if (event.recid == "") {
                        ...
                        $.ajax({
                            ...
                            async: false,
                            data: { "cmd": 'validaTraeFVto', "codMaterial": local_Codigo, "Bodega": w2ui['form'].record['Nombre_Bodega'], "Nserie": event.value_new.toUpperCase() },
                            dataType: "json",
                            success: function (response) {
                                ...
                                if (response.validate == "1") {
                                    ...
                                    local_Existencia = parseInt(response.FLD_MOVCANTIDAD);
                                    local_Fecha = response.FLD_FECHAVENCIMIENTO;
                                } // Fin Validador de codigo
                        ...
                        if (local_Fecha == "" || local_Total == "") {
                            w2ui['grid'].add({ recid: event.recid, codMaterial: local_Codigo, nombreMaterial: local_Nombre, item: local_Item, cantidad: local_Cantidad, valor: local_Valor, total: local_Total, existencia: local_Existencia, loteSerie: local_Serielote, fechaVencimiento: local_Fecha });
                        } else {
                            w2ui['grid'].add({ recid: event.recid, codMaterial: local_Codigo, nombreMaterial: local_Nombre, item: local_Item, cantidad: local_Cantidad, valor: local_Valor, total: local_Total, existencia: local_Existencia, loteSerie: local_Serielote, fechaVencimiento: local_Fecha });
                            w2ui['grid'].add({ recid: '', codMaterial: '', nombreMaterial: '', item: '', cantidad: '', valor: '', total: '', loteSerie: '', fechaVencimiento: '' });
                        }
            ...
            // ================================
            // Grabar nueva Donación, saveForm
                    ...
                    if (w2ui['grid'].records.length > 0 && w2ui['form'].record['Nombre_Bodega'] && w2ui['form'].record['Nombre_Proveedor']) {
                        ...
                        if (local_Fecha == "" || local_Serie == "") {
                            ...
                            for (var i = 0; i <= w2ui['grid'].records.length - 1; i++) {
                                ...
                                $.ajax({
                                    type: "POST",
                                    url: "../../clases/persistencia/controladores/Despachos/PorCanjeDespacho/saveDetalleMov_DespxCanje.ashx",
                                    async: false,
                            ...
                            $.ajax({
                                ...
                                async: false,
                                data: { "fecha": w2ui['form'].record['anioDonacion'], "NumeroDonacionArticulo": 'C', "NumeroCorrelativo": w2ui['form'].record['NDonacion'], "fechaCompleta": w2ui['form'].record['fechaServidor'], "descripcion": w2ui['form'].record['descripcion'].toUpperCase(), "valor": total, "rut": RutSinDigito, "digitoRut": digito },
                                dataType: "json",
                                success: function (response) {
